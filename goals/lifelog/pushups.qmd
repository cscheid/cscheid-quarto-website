...
```{ojs}
domain = (d) => [Math.min(...d), Math.max(...d)]
```
...
deficit = Plot.plot({
    ...
    y: {
        domain: domain([0, ...df.map(d => d.deficit)]),
        grid: true
    },
    marks: [
...
weeklyAverage = Plot.plot({
    ...
    y: {
        domain: domain([0, ...df.map(d => d.daily_avg_prev_week)]),
        grid: true
    },
    marks: [
...
```{ojs}
remainderPerDayData = df.map(x => ({...x, remainder_per_day: (24 * 365 - x.total) / (365 - x.days)}))
remainderPerDay = Plot.plot({
    x: {
        domain: [0, 365]
    },
    y: {
        domain: domain(remainderPerDayData.map(x => x.remainder_per_day)),
        grid: true
    },
    marks: [
        Plot.line(remainderPerDayData, {x: "days", y: "remainder_per_day"})
    ]
})
```
