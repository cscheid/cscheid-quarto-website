...
```{ojs}
deficitYDomain = [Math.min(0, ...df.map(d => d.deficit)), 1600]
deficit = Plot.plot({
    x: {
        domain: [0, 365]
    },
    y: {
        domain: deficitYDomain,
        grid: true
    },
    marks: [
        Plot.line(df, {x: "days", y: "deficit"})
    ]
})
```
...
```{ojs}
weeklyAverageYDomain = undefined
weeklyAverage = Plot.plot({
    x: {
        domain: [0, 365]
    },
    y: {
        domain: weeklyAverageYDomain,
        grid: true
    },
    marks: [
        Plot.line(df, {x: "days", y: "daily_avg_prev_week"})
    ]
})
```
...
```{ojs}
yearlyAverageYDomain = undefined
yearlyAverage = Plot.plot({
    x: {
        domain: [0, 365]
    },
    y: {
        domain: yearlyAverageYDomain,
        grid: true
    },
    marks: [
        Plot.line(df, {x: "days", y: "daily_avg_total"})
    ]
})
```


## Avg per day going forward

```{ojs}
remainderPerDayYDomain = [24, 30]
remainderPerDay = Plot.plot({
    x: {
        domain: [0, 365]
    },
    y: {
        domain: remainderPerDayYDomain,
        grid: true
    },
    marks: [
        Plot.line(df.map(x => ({...x, remainder_per_day: (24 * 365 - x.total) / (365 - x.days)})), {x: "days", y: "remainder_per_day"})
    ]
})
```
...
print("Average per day so far: %.3f\n\n" % avg_so_far)
```
